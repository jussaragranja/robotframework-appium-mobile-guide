...
O ADB é a ferramenta de linha de comando que permite se comunicar com dispositivos e emuladores Android.

### 1. Verificar se o ADB está instalado

```bash
adb version
```

- Se aparecer a versão, o ADB já está disponível.  
- Se o comando não for encontrado, instale os **Platform Tools**.

<details>
<summary><strong>2. Instalar ADB (Platform Tools)</strong></summary>

### Windows

1. Baixe os **Android Platform Tools** em:  
   https://developer.android.com/studio/releases/platform-tools  
2. Extraia o conteúdo para uma pasta (ex.: `C:\Android\platform-tools`).  
3. Adicione essa pasta ao **PATH** do sistema.

✅ Feche e abra o terminal novamente e verifique:
...
Depois, adicione ao PATH (exemplo usando bash/zsh):

```bash
export PATH=$PATH:/caminho/para/platform-tools
```

Você pode adicionar essa linha ao `~/.bashrc` ou `~/.zshrc` para ficar permanente.
